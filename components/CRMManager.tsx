...
const CRMManager: React.FC<CRMManagerProps> = ({ currentUser, onConvertToProject }) => {
  ...
  useEffect(() => {
    ...
    const unsubInbox = onSnapshot(query(lineConnectionsCollection, orderBy("timestamp", "desc")), 
      (snap) => setLineInbox(snap.docs.map(d => ({ id: d.id, ...d.data(), lineUserId: d.id } as LineConnection)).filter(i => !i.isBound)),
        ...
        <div className="max-w-2xl mx-auto space-y-6">
          <div className="bg-white p-8 rounded-[40px] border border-slate-200 shadow-xl">
            ...
            <div className="space-y-4">
              {lineInbox.map(item => (
                <div key={item.id} className="p-5 bg-slate-50 rounded-3xl border border-slate-100 flex items-center justify-between group">
                  <div className="flex items-center gap-4">
                    <img src={item.linePictureUrl} className="w-14 h-14 rounded-2xl" />
                    <h4 className="font-black text-slate-800">{item.lineDisplayName}</h4>
                  </div>
                  <button onClick={() => setLinkingItem(item)} className="bg-[#06C755] text-white px-5 py-3 rounded-2xl font-black text-sm flex items-center gap-2"><LinkIcon className="w-4 h-4" /> 綁定</button>
                </div>
              ))}
              {lineInbox.length === 0 && <div className="text-center py-20 text-slate-300 font-bold italic">目前無人敲敲門...</div>}
            </div>
          </div>
        </div>
      )}
